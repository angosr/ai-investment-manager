...
class CapitalDashboardReader:
    ...
    def overview(self, *, now: datetime | None = None) -> CapitalOverview:
        ...
        with self._engine.connect() as connection:
            ...
            cycle_record = self._latest_payload(
                ...
                CapitalCycleRecord,
                where_clause=(capital_cycle_records.c.pipeline_id == self._config.capital.version),
            )
            ...
            order_count = int(
                connection.scalar(
                    select(
                        func.count(func.distinct(product_order_observations.c.client_order_id))
        ...
        for symbol in sorted(specs_by_symbol):
            ...
            spec = next(
                (
                    item
                    for item in specs
                    if position is not None and item.instrument.key == position.instrument.key
                ),
                next(
                    (item for item in specs if item.instrument.product == InstrumentProduct.SPOT),
                    specs[0],
                ),
            )
            ...
            rows.append(
                CapitalInstrumentStatus(
                    ...
                    quote_observed_at=(None if valuation is None else valuation.observed_at),
    ...
    def _policy_status(self) -> CapitalPolicyStatus:
        ...
        return CapitalPolicyStatus(
            ...
            reference_policy_version=(
                None if capital.reference_policy is None else capital.reference_policy.version
            ),
        )
        ...
        *,
        ...
        return tuple(
            CapitalEquityPoint(
                ...
                net_pnl=(account.accounting.net_pnl if account.accounting is not None else None),
                drawdown_fraction=account.drawdown_fraction,
                cash_benchmark_equity=(baseline.equity if baseline is not None else None),
                increment_vs_cash=(
                    account.equity - baseline.equity if baseline is not None else None
                ),
            )
    ...
    def activity(
        ...
        *,
        ...
        with self._engine.connect() as connection:
            ...
            projections = (
                {
                    item.projection_id: ProductPayoffProjection.model_validate(item.payload)
                    for item in connection.execute(
                        select(
    ...
    def _position_changes(
        *,
        ...
        planned_legs = {item.leg_id: item for group in plan.groups for item in group.legs}
            ...
            for leg in (*group.target_legs, *group.compensation_legs):
                ...
                if leg.role == ExecutionLegRole.COMPENSATION:
                    ...
                    if planned.reduce_only:
                        ...
                        operation = "CLOSE" if delta.desired_gross_notional == 0 else "REDUCE"
                    else:
                        operation = "OPEN" if delta.current_gross_notional == 0 else "INCREASE"
    ...
    def _candidate_economics(
        ...
        *,
        ...
        for evaluation in target.candidate_evaluations:
            ...
            candidate_target = forecast.target if projection is None else projection.target
            candidates.append(
        ...
        "instruments": [
            {
                ...
                "average_price": (None if item.average_price is None else str(item.average_price)),
                ...
                "quote_quality": (None if item.quote_quality is None else item.quote_quality.value),
            }
        ...
        "position_changes": [
            {
                ...
                "average_fill_price": (
                    None if change.average_fill_price is None else str(change.average_fill_price)
                ),
                "fee": str(change.fee),
            }
        ...
        "points": [
            {
                ...
                "cash_benchmark_equity": (
                    None if item.cash_benchmark_equity is None else str(item.cash_benchmark_equity)
                ),
                "increment_vs_cash": (
