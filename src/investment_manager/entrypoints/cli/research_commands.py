...
@app.command("evaluate-fixed-maturity-carry")
def evaluate_fixed_maturity_carry_command(
    config: Annotated[Path, typer.Option(exists=True, dir_okay=False)],
    plan: Annotated[Path, typer.Option(exists=True, dir_okay=False)],
    spot_dataset_id: Annotated[str, typer.Option()],
    project_root: Annotated[Path, typer.Option(exists=True, file_okay=False)] = Path(
    ...
    dataset_catalog: Annotated[Path, typer.Option(file_okay=False)] = Path(
        ".runtime/fixed-maturity-carry-datasets"
    ),
    result: Annotated[Path, typer.Option(dir_okay=False)] = Path(
        "evidence/quant-candidates/btc-quarterly-cash-carry-v1-result.json"
    ),
) -> None:
    """冻结并一次性评价预注册的 BTC 固定到期 carry；不能直接授权资本。"""
    ...
    from investment_manager.research.fixed_maturity_carry import (
        ...
        fetch_fixed_maturity_carry_dataset,
        load_fixed_maturity_carry_plan,
        store_fixed_maturity_dataset,
        store_fixed_maturity_result,
    )

    root = project_root.resolve()
    loaded = load_config(config)
    registered = load_fixed_maturity_carry_plan(plan)
    ...
    dataset_path = store_fixed_maturity_dataset(dataset, root=dataset_catalog)
    evaluated_at = datetime.now(UTC)
    evaluation = evaluate_fixed_maturity_carry(
        plan=registered,
        spot_dataset=spot,
        dataset=dataset,
    ...
    result_path = store_fixed_maturity_result(evaluation, target=result)
    typer.echo(
        json.dumps(
            {
                ...
                "official_contract_count": evaluation.official_contract_count,
                "complete_contract_count": evaluation.complete_contract_count,
                "entered_contract_count": evaluation.entered_contract_count,
                "net_return_fraction": str(evaluation.net_return_fraction),
                "maximum_account_drawdown_fraction": str(
                    evaluation.maximum_account_drawdown_fraction
                ),
                "minimum_futures_margin_buffer_fraction": (
                    None
                    if evaluation.minimum_futures_margin_buffer_fraction is None
                    else str(evaluation.minimum_futures_margin_buffer_fraction)
                ),
