...
@app.command("portfolio-momentum-walk-forward")
def portfolio_momentum_walk_forward_command(
    database_url: Annotated[
        str,
        typer.Option(envvar="INVESTMENT_MANAGER_DATABASE_URL", help="EvaluationPlan 事实库"),
    ],
    btc_dataset_id: Annotated[str, typer.Option()],
    eth_dataset_id: Annotated[str, typer.Option()],
    plan_id: Annotated[str, typer.Option()],
    catalog: Annotated[Path, typer.Option(exists=True, file_okay=False)] = Path(
        ".runtime/datasets"
    ),
    evaluation_catalog: Annotated[Path, typer.Option(file_okay=False)] = Path(
        ".runtime/portfolio-momentum-evaluations"
    ),
    training_days: Annotated[int, typer.Option(min=365)] = 730,
    test_days: Annotated[int, typer.Option(min=90)] = 365,
    starting_equity: Annotated[str, typer.Option()] = "10000",
    minimum_trades: Annotated[int, typer.Option(min=1)] = 60,
    minimum_profit_factor: Annotated[str, typer.Option()] = "1.05",
    minimum_daily_return_bps_lower_bound: Annotated[str, typer.Option()] = "0",
    maximum_drawdown_fraction: Annotated[str, typer.Option()] = "0.05",
    minimum_positive_fold_fraction: Annotated[str, typer.Option()] = "0.75",
    include_trades: Annotated[bool, typer.Option()] = False,
    register_only: Annotated[
        bool,
        typer.Option(help="只预登记固定 BTC/ETH 组合实验，不运行回测"),
    ] = False,
) -> None:
    """预登记或评价固定 BTC/ETH 中频组合动量；历史已暴露，不授予交易资格。"""

    from investment_manager.research.dataset import HistoricalDatasetCatalog
    from investment_manager.research.portfolio_momentum import (
        PortfolioMomentumEvaluationCatalog,
        ...
        dataset_identities,
        failed_portfolio_momentum_experiment,
    ...
    try:
        parsed_equity = Decimal(starting_equity)
        parsed_profit_factor = Decimal(minimum_profit_factor)
        parsed_lower_bound = Decimal(minimum_daily_return_bps_lower_bound)
        parsed_drawdown = Decimal(maximum_drawdown_fraction)
        parsed_positive_folds = Decimal(minimum_positive_fold_fraction)
    except InvalidOperation as exc:
        raise typer.BadParameter("组合动量评价门槛必须是十进制数") from exc
    if (
        parsed_equity <= 0
        or parsed_profit_factor <= 0
        or not Decimal("0") < parsed_drawdown <= Decimal("1")
        or not Decimal("0") < parsed_positive_folds <= Decimal("1")
    ):
        raise typer.BadParameter("权益和盈利因子必须为正，比例门槛必须在 (0,1] 内")

    datasets_catalog = HistoricalDatasetCatalog(catalog)
    datasets = (
        datasets_catalog.load(btc_dataset_id),
        datasets_catalog.load(eth_dataset_id),
    )
    if tuple(item.manifest.symbol for item in datasets) != ("BTCUSDT", "ETHUSDT"):
        raise typer.BadParameter(
            "btc-dataset-id 与 eth-dataset-id 必须按 BTCUSDT、ETHUSDT 顺序提供"
        )
    policy = PortfolioMomentumPolicy()
    walk_forward_plan = PortfolioMomentumWalkForwardPlan(
        plan_id=plan_id,
        ...
        minimum_trades=minimum_trades,
        minimum_profit_factor=parsed_profit_factor,
        minimum_daily_return_bps_lower_bound=parsed_lower_bound,
        maximum_drawdown_fraction=parsed_drawdown,
        minimum_positive_fold_fraction=parsed_positive_folds,
    )
    code_version = current_clean_code_version()
    ...
    if registered is None:
        raise typer.BadParameter(
            "EvaluationPlan 尚未预登记；先以相同参数执行 --register-only",
            param_hint="plan-id",
        )
    try:
        validate_portfolio_momentum_evaluation_plan(
    ...
    result_path = PortfolioMomentumEvaluationCatalog(evaluation_catalog).store(result)
    if not result.passed:
        governance.record_failed_experiment(
            failed_portfolio_momentum_experiment(
                result,
    ...
    if not include_trades:
        for fold in payload["folds"]:
            fold["run"].pop("trades", None)
            fold["run"].pop("daily_net_returns", None)
    typer.echo(json.dumps(payload, ensure_ascii=False, indent=2))
