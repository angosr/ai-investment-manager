...
class _PublicationParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        tag = tag.lower()
        self._capture_attributes(tag, attrs)
        if tag == "main" and self._main_depth == 0:
            self._main_depth = 1
            return
        if self._main_depth and tag not in self._VOID_TAGS:
            self._main_depth += 1
            if tag in self._HIDDEN_TAGS:
                self._hidden_depth += 1
            if tag == "h1":
                self._heading_depth = self._main_depth
    ...
    def handle_startendtag(
        ...
        self._capture_attributes(tag.lower(), attrs)
    ...
    def handle_endtag(self, tag: str) -> None:
        tag = tag.lower()
        if not self._main_depth or tag in self._VOID_TAGS:
            return
        if tag in self._HIDDEN_TAGS and self._hidden_depth:
            self._hidden_depth -= 1
        if self._heading_depth == self._main_depth and tag == "h1":
            self._heading_depth = 0
        self._main_depth -= 1

    def handle_data(self, data: str) -> None:
        text = " ".join(data.split())
        if not text or not self._main_depth or self._hidden_depth:
            return
    ...
    @property
    def title(self) -> str:
        return self.meta_title or " ".join(self._heading_parts).strip()
