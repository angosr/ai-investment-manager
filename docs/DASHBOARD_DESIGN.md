...
按相同结果归并；“AI”只展示通过中文和输出完整性门禁的现役 `ContextAssessment`，避免高频分析记录
淹没实际资本行动；同时单独显示最近一次 AI 尝试、最近有效判断、当前行为近 24 小时的质量拒绝次数，以及
按当前规则被排除的历史记录数量。无效正文不进入判断列表，但质量失败不得被伪装成“没有发生”；
