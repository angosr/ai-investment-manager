...
门禁。未形成合法分析契约的调用没有 Assessment 正文，只显示最近一次调用状态、当前行为近 24 小时的契约
...
不可变 `ContextAssessment`，包括联合判断、可证伪机制、事件引用及影响状态、竞争证据、程序验证和失效条件。WorldModel 不直接展示资本结论；哪些机制真正影响了概率、完整成本和最终现金/仓位取舍，只在对应 Capital Forecast 行动中展示，不能用相邻布局暗示世界认知已经改变仓位。
最新世界认知是最新合法 Assessment 的直接投影，不能改名为“市场判断”，也不能由附近新闻重新拼装引用。
“世界事件”直接按发生时间投影权威事实库中的新闻、一手事实与运行触发；唯一身份在写入时已去重，页面不再跨库合并或猜测对应关系。
