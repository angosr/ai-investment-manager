...
联合 Forecast 的最小不可变内容是：共同截止与时域、成员经济暴露及规范参考 Outcome、每个联合情景的概率与成员收益、来源/行为身份、输入引用和缺失结果。边际预期、协方差和尾部损失从这份联合分布确定性派生，不另外维护一套可漂移数字。每项产品投影另绑定 Instrument、可成交锚点、参考价格映射、basis/funding/现金流假设、映射不确定性和产品规则引用；缺少任一必要状态时该 Instrument 没有候选，而不是让 AI 补猜。Forecast 中的多腿只允许表达一个不可拆分 payoff，例如需要原子执行和共同结算的价差；它不能用预设 `gross_weight` 偷渡资产配置，普通多资产权重始终由 Portfolio 决定。

Portfolio 以当前总财富为统一分母，只读取 Forecast 已形成的产品 payoff 投影，选择使保守的费用后长期增长最优、同时满足尾部和生存约束的产品有符号目标暴露；它不能自行把经济观点翻译为未验收产品。实现可以采用约束均值—方差、情景效用或其他经样本外证据胜出的算法，但都必须显式包含：预期收益估计误差、产品映射误差、协方差或共同尾部、现有持仓、经济现金、全量未来成本、可用流动性、funding、保证金和换手。Reference 可以让各项经济暴露与经济现金合计为 100%，但现金不是交易 Leg，也不等于产品账户的原始现金余额；产品编译根据同一目标分别产生 Spot 资金占用、Derivative 名义和保证金，并把衍生品的融资储备在资本配置上归属到对应风险暴露，避免再次算作中性现金。这种归属不会消除该储备真实承担的结算资产、托管、脱锚和压力流动性风险；Risk 仍按实际资产和抵押状态计量，不用战略归属抵消。优化器不能把一个微小而不确定的均值差放大成极端集中度；简单分散、最小方差或 Reference Policy 必须作为同风险基线参与竞争。
