...
Mandate 中无法从市场数据推导的偏好必须来自资产所有者：可追加或提取的现金、最低流动性、真实购买力参照、可接受的永久损失和回撤、杠杆与法律限制。Agent 可以指出目标内部矛盾、提出更稳健的候选并运行隔离模拟，但不能根据近期行情、模型输出或为了产生订单静默改写这些边界。信息尚不完整时只能登记明确版本的临时 Mock Mandate；它可以用来验证全尺寸模拟经济性，不能冒充正式资本授权。Mandate 不是频繁优化参数，只有资产所有者约束改变或长期证据推翻其实现假设时才建立新版本。
...
Portfolio 以当前总财富为统一分母，选择使保守的费用后长期增长最优、同时满足尾部和生存约束的目标权重。实现可以采用约束均值—方差、情景效用或其他经样本外证据胜出的算法，但都必须显式包含：预期收益估计误差、协方差或共同尾部、现有持仓、现金、全量未来成本、流动性、funding、保证金和换手。优化器不能把一个微小而不确定的均值差放大成极端集中度；简单分散、最小方差或 Reference Policy 必须作为同风险基线参与竞争。
...
仓位尺度来自整个账户的目标权重和风险贡献，不来自固定“单次交易金额”。总风险包络先限定可承受的组合波动、压力损失、回撤、gross/net、保证金和流动性，再由 Portfolio 在包络内分配目标；Risk 只负责缩减越界结果。固定美元上限只能是某个实验的资本授权，不能成为长期 sizing 公式。最小订单金额和最小再平衡差只用于避免无法成交或成本不经济的碎片订单。

任一订单的经济金额只能由 `总权益 ×（目标权重 - 当前权重）` 经产品乘数、汇率、可成交深度和取整得到。模拟盘与正式盘在 Portfolio 层使用同一目标权重，因此“模拟”本身不能把应有的 10% 或 20% 账户暴露缩成几十 USDT；几十 USDT 只可能是明确登记的技术 Canary，不得进入全尺寸策略收益评价。反过来，账户规模较小也不构成满仓或放大杠杆的理由：若风险预算只允许 3%，正确仓位就是 3%；是否有经济意义由预期净改善相对估计误差和完整成本决定，而不是由用户期望的订单观感决定。
...
Portfolio 每次从当前权重向目标权重交易，而不是清仓后重建。长期先验、周期状态和突发事件可以按各自原生时域更新，但只能在投影到同一资本决策时域后共同改变这一个目标。只有当目标变化的保守收益改善超过完整转换成本和不确定性缓冲，或现有风险越界时才再平衡；交易发生在经济无交易区间被突破时，不由固定日历或订单次数驱动。
...
Reference Policy 的构造本身必须简单、透明且不使用主动 WorldModel：它先把 Instrument 映射到非重复经济风险，再以 mandate 的时域、流动性和总风险边界决定中性暴露，并以预登记权重带或成本阈值再平衡。固定权重、风险平衡、最小方差等只是候选，不预设任何一种必然正确；应选择在点时盲测中最稳健、最少参数且真实可执行的一种，按版本冻结后才能成为主基准。Mandate 的允许暴露是法律与所有者边界，不是必须凑齐的资产清单；最终 Reference 可以稀疏，但必须由共同压力情景、风险贡献和费用后购买力结果证明与总风险匹配，不能用产品数量或人工 `eligible` 标签自证。它不能随最新 AI 观点或近期表现变化，否则既失去基准含义，也与实际 Portfolio 重叠。
...
选择制品在看结果前冻结数据截止、代理映射、候选规则、完整成本、共同压力情景、开发/盲测区间和否决条件；结果保存每个候选的费用后增长、流动性、换手、回撤、尾部损失、风险贡献与参数稳定性，并只允许一个胜出版本。发布时 Release 必须按内容哈希引用该制品，不能只在配置里填写权重。Reference 的反事实账户随后复用正式账户的点时估值、产品现金流、取整和成本语义机械推进；只有权重带被成本后的风险偏离突破才再平衡，不按月、固定日期或 AI 观点变化交易。若没有候选通过，`Reference Policy` 保持为空，总组合主实验明确未启动；现役单产品组件实验可以继续，但不得宣称已经具备总账户资产管理或相对主动增值证据。
...
资本结果至少按三层归因：Reference Policy 自身的市场风险溢价、Forecast/Portfolio 带来的主动偏离、Risk/Execution/成本造成的保留或损耗。主结论读取相对 Reference Policy 的费用后增值及其跟踪风险、回撤和尾部结果；绝对 PnL、相对现金、单资产被动暴露和 Forecast 分数作为互补诊断，不能选择表现最好的一条事后改名为基准。
