...
单次决策正确计入未来成本仍不足以证明整条政策有效，因为相邻 Forecast 翻转、重复消费和连续小幅调整会在跨周期累计费用。Execution Evaluation 必须从终态成交组唯一重建每个新增暴露 lot 到退出的往返，冻结其开平 cycle、持有时间、gross turnover、方向毛收益、费用和净收益，并汇总“毛收益为正但费用后为负”的成本反转；最近新增暴露采用 LIFO 只作 churn 因果诊断，不改账户会计。该结果必须与账户累计价格收益和费用在全平时核对，用来定位 Forecast、Capital 或 Execution 的首个失效层。它本身不下单，也不以任意费用比例动态改写同一 cohort；停止或替换政策只能读取事前登记的总资本评价与损失边界并作用于未来行为。
