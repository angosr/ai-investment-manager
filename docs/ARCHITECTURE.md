...
选择计划在运行 evaluator 前以内容身份冻结：唯一候选、经济代理、开发/盲测半开区间、产品证据的最小数量与时间覆盖、完整成本版本、共同压力窗口以及购买力、回撤、压力、换手、成本和风险贡献阈值。当前版本的长期经济代理必须固定到准确数据集哈希；仍在增长的产品 K 线、双边报价、funding 和规则证据则冻结作用域与最低覆盖，评价制品再绑定实际命中的唯一数据集。拒绝记录只能产生不可变 `REJECTED`：它同时保存已可计算的经济失败和产品证据缺口；只有两者都无拒绝理由时才必须进入独立费用后 evaluator，不能由准备度检查直接授予资格。结果保存开发、盲测、逐个压力窗口、费用后购买力、换手、回撤和风险贡献，并只登记一个合格版本。发布时 Release 不仅校验文件哈希，还必须解析制品，确认其计划内容、证据层、状态、唯一候选、目标暴露和再平衡带与配置完全一致；被拒绝、缺证据或只同名的文件不能授予资格。Reference allocation 的 `target_exposure_fraction` 表达经济暴露比例，经济现金可以作为显式剩余项使其合计为一；编译后的产品名义、账本现金、抵押和可用流动性是不同投影，不能再要求彼此合计为一。其反事实账户随后复用正式账户的点时估值、产品现金流、取整和成本语义机械推进；只有暴露带被成本后的风险偏离突破才再平衡，不按月、固定日期或 AI 观点变化交易。若候选被拒绝，`Reference Policy` 保持为空，总组合主实验明确未启动；现役单产品组件实验可以继续，但不得宣称已经具备总账户资产管理或相对主动增值证据。
