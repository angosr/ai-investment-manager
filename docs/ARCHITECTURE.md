...
模拟实验只保留从真实经济边界派生的约束：账户级回撤与压力损失、最低可用流动性、gross/net 与保证金生存边界、产品规则、可成交深度和完整未来成本。仓位由保守费用后边际、估计不确定性、共同尾部与当前组合连续求得，不由每个 Forecast 上的固定份额或固定阈值决定。Portfolio 不再设置独立的固定最小调仓金额；数量步长和最小订单金额只由当时适用的产品执行规格裁决，避免同一边界维护两次。固定 Forecast cadence 不得长于新建仓有效窗，否则每轮之间会形成没有经济依据的资本资格空档；预测时域可以长于 cadence，重叠 Outcome 由 Evaluation 按时间簇处理。模拟盘不得强制负边际交易来凑订单；每个合法多空产品投影无论是否被 Portfolio 选中都必须在共同终点结算，因此未成交候选同样形成反事实反馈。若实际订单长期为零，系统应能用这些结算区分“候选确实没有净优势”和“预测、产品或资本链失效”。
