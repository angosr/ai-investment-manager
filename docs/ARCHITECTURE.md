...
每份产品投影无论是否被 Portfolio 选择，都从自身冻结的可成交入场锚点结算到源经济 Forecast 的共同终点；Spot 使用退出侧报价，Derivative 同时结算真实 funding，点时行情或资金费率在宽限期后仍不完整就永久记为不可用。产品映射评价只在每个源 Forecast 的第一次完整资本决策截面比较各合法表达，并对重叠经济时域去相关；后续持仓复核投影仍永久保存和结算，但不能因触发更频繁而在主证据中获得更高权重。评价至少报告点估计误差、保守包络覆盖、收益符号、产品排序命中和相对事后最佳合法表达的后悔值。这里检验的是产品映射，不把多个产品重新计为多个 Forecast；手续费、点差、滑点和真实选择相对现金/Reference 的增量仍只由 Portfolio 资本评价负责。
