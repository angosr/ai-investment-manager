...
初始 Quant 实验从一个简单、透明、可完全回放的指定模型和少量有经济含义的因子开始；momentum、mean reversion、carry、breakout、volatility 与 liquidity 只有在当前目标、时域和数据点时语义完整时才进入候选。开发、验证和盲测按时间顺序冻结，所有搜索过的候选和失败结果计入选择历史；不得因为复杂模型历史拟合更好就默认组合或上线。常规市场状态识别、模型适用度和候选权重必须由冻结程序根据点时数据确定，AI 不得在每次调用中离散选择或启停算法；否则同一输入的生成波动会被伪装成择时能力，并直接制造换手。指定 prior 可以是经盲测胜出的单模型，也可以是稀疏、稳定且可解释的条件混合，但二者必须作为同一种 Quant 制品接受样本外竞争，而不是并存为两套决策链。某目标缺少合格点时历史时输出 `NO_ESTIMATE`，不得用静态概率、其他产品 K 线或当前已知未来补齐成伪 Quant 预测。
