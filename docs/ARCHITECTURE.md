...
这条边界由三个可执行不变量共同保证，而不是目录命名约定：受管 Release 使用的生产 CLI 导入图不得到达 `research` 或 `legacy`；`AppConfig` 与 Release 组件版本不得出现只被离线实验消费的策略、校准、频率、旧风险或旧执行政策；研究命令使用独立入口和独立 `ResearchConfig`。历史表可以继续登记在 schema 中，但只能由明确的只读历史投影或一次性离线研究读取，不能因此把退役模型重新装入生产进程。
