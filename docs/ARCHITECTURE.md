...
Reference Policy 的构造本身必须简单、透明且不使用主动 WorldModel：它先把 Instrument 映射到非重复经济风险，再以 mandate 的时域、流动性和总风险边界决定中性目标暴露，并以预登记暴露带或成本阈值再平衡。固定暴露、风险平衡、最小方差等只是候选，不预设任何一种必然正确；点时盲测和跨状态压力只负责否决不稳健、不可执行或参数敏感的候选，不能按历史收益排名挑一个“赢家”。Reference 是资产所有者选择的中性风险设定，不是 Alpha；多个候选都满足 Mandate 时，按事前冻结的简单性、成本、流动性和参数稳定性顺序选择最朴素者，而不是最高回测收益者。Mandate 的允许暴露是法律与所有者边界，不是必须凑齐的资产清单；最终 Reference 可以稀疏，但必须由共同压力情景、风险贡献和费用后购买力结果证明与总风险匹配，不能用产品数量或人工 `eligible` 标签自证。它不能随最新 AI 观点或近期表现变化，否则既失去基准含义，也与实际 Portfolio 重叠。
...
选择制品在看结果前冻结数据截止、代理映射、候选规则、完整成本、共同压力情景、开发/盲测区间、否决条件和确定性择简顺序；结果保存每个候选的费用后增长、压力后流动性、换手、回撤、尾部损失、风险贡献与参数稳定性，并只登记一个合格版本。发布时 Release 不仅校验文件哈希，还必须解析制品，确认其计划内容、证据层、状态、唯一登记候选、目标暴露和再平衡带与配置完全一致；被拒绝、缺证据或只同名的文件不能授予资格。Reference allocation 的 `target_exposure_fraction` 表达经济暴露比例，经济现金可以作为显式剩余项使其合计为一；编译后的产品名义、账本现金、抵押和可用流动性是不同投影，不能再要求彼此合计为一。其反事实账户随后复用正式账户的点时估值、产品现金流、取整和成本语义机械推进；只有暴露带被成本后的风险偏离突破才再平衡，不按月、固定日期或 AI 观点变化交易。若没有候选通过，`Reference Policy` 保持为空，总组合主实验明确未启动；现役单产品组件实验可以继续，但不得宣称已经具备总账户资产管理或相对主动增值证据。
