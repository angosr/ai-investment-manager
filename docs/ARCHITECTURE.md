...
Portfolio 不建立“被动核心仓”“AI 仓”“多头策略仓”和“空头策略仓”等平行资本池。长期风险溢价模型形成 Forecast 的中性收益先验，WorldModel 和其他已验证信号只对先验形成有期限、有不确定性的条件修正；Reference Policy 则独立表达 mandate 允许的总风险及低成本可投资基准。三者各有不同语义，但正式资本最终只进入同一个 PortfolioTarget。没有可信主动观点时，目标自然收缩到 Reference Policy 或结算余额，而不是要求 AI 为每项持仓持续制造方向理由。
...
账户较小时，多资产不意味着机械均分或持有每一类资产。固定成本、最小订单和估计误差会使最优组合保持稀疏：只建立少数能显著改善总组合风险收益的暴露，每项金额仍由总权益、边际风险贡献和不确定性决定。对线性产品，若建议增量小到无法在完整成本后对总组合收益或风险产生可辨识改善，就把该增量归零，而不是为了展示“有交易”保留几十 USDT 的装饰性仓位；真正有价值的暴露应按目标风险获得有经济意义的账户权重。方向同样不是目标：做多用于承担有报酬的风险，做空用于表达经验证的负收益或降低组合风险；做空产品若在 funding、basis、强平和场所风险后不改善总组合，就保持现金或减仓。
...
2. 建立唯一 Mandate/可投资域与 Reference Policy，使数据、产品语义、风险包络和主基准先成为同一版本事实；
...
- CPP Investments 的[投资政策与总风险目标](https://www.cppinvestments.com/wp-content/uploads/2025/04/InvestmentStatement-InvestmentPortfolio-EN-Apr022025.pdf)使用可投资 Reference Portfolio 表达长期绝对风险目标，并在总组合层管理因子、流动性和杠杆。本设计采用“一个总组合、一个风险匹配主基准”，但不照搬其规模、杠杆或资产权重。
