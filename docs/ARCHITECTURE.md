...
上下文窗口。Packet 先保证直接变化与异常候选；同一证据层级内先各取一种事实类型，再取同类第二条，
避免连续日程行挤出收益率、美元、财政现金或资金流等传导证据。总容量裁剪沿相反顺序移除，
...
覆盖缺口和真正入选的证据。观测台的“AI 输入快照”使用同一高密度投影，不能把审计字段冒充模型输入。

`ContextAssessment` 的最小论证合同只有四部分，避免另建宏大类型系统：

1. 当前是否存在足以改变基准情景的主导驱动、作用方向与置信边界，以及已获证据支持的传导链；不能把
   涨跌、趋势或区间本身当作原因，也不能用脱离当前证据的通用经济学链条填充；
