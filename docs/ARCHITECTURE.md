...
实验资本不是为了制造订单而强制交易。每个有效 Forecast 都必须生成正向表达、反向表达与现金的费用后比较；只有某一有符号表达在 mapping uncertainty、funding、点差、手续费、滑点、保证金和组合共同尾部之后仍有正边际，Portfolio 才建立具有账户意义的目标暴露。三个暴露都必须持续产生 Forecast/NoEstimate、产品结果和同口径逻辑账户结果；只有相对基线形成费用后前瞻增量的唯一 producer 才获得有界模拟资本权限。现行 Context producer 的新增资本权限已因负收益、换手成本和不稳定性撤销，Quant 与 AI + Quant 保持研究态，既有正式模拟持仓只按原 Forecast 血缘维持或退出。长期零交易若来自持续无净边际是投资结论，若来自研究 Forecast、产品投影或逻辑账户链失效则是系统缺陷，健康状态必须明确区分。
