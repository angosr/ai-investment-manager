...
运行幂等与模型稳定性是两件事。同一决策槽只保存一次权威 Forecast，重试不能更换交易结果；但这不能证明生成模型对同一输入不敏感。候选行为还必须在不进入资本的精确输入副本上测量概率分布离散度、期望收益离散度，以及经过同一产品映射与成本后方向或现金选择的翻转率。副本调用分别审计，但不增加 Forecast 样本、Outcome 数或投票权，也不默认变成实时多 Agent 集成；若生成噪声与候选费用后 Edge 同量级，该 Edge 不具备新增资本的可识别性，仓位必须按实证不确定性收缩或由该行为失去资本资格。只有前瞻准确性、费用后结果和同输入稳定性共同支持时，复杂推理才优于简单基线。
