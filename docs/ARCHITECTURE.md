...
生产运行、离线研究和历史可读性共享不可变领域事实，但不共享决策入口。生产配置只包含现役闭环所消费的政策；研究策略、筛选、回测和旧校准使用独立研究配置，生产 `AppConfig`、Service、Dashboard 与 Governance 均不得为了让旧回放继续工作而加载它们。研究可以读取领域模型和匿名化历史事实，生产代码不得导入 `research`。

这条边界由三个可执行不变量共同保证，而不是目录命名约定：受管 Release 使用的生产 CLI 导入图和运行时导入闭包均不得到达 `research` 或 `legacy`；`AppConfig` 与 Release 组件版本不得出现只被离线实验消费的策略、校准、频率、旧风险或旧执行政策；研究命令使用独立入口和独立 `ResearchConfig`。托管运行 Schema 只组合现役领域所有者，发布预检和临时装配不得注册退役表。历史表可以继续存在于物理迁移目录，并由显式离线 Schema 读取，以保存不可变事实和验证迁移；这不赋予旧模型、Repository 或 Worker 生产身份。
...
退役行为的数据库事实永久保留，但“可查询”不等于“仍运行”。Dashboard 可以通过只读投影展示历史行动；Outcome 可以完成迁移切流时已经登记且尚未到期的终结义务。除此之外，不保留旧 Producer、周期编排、订单入口、定时结算器、通用窗口评价或自动提案轮询。旧表的读取合同迁入拥有该历史事实的现役领域后，`legacy` 模块本身删除，不建立兼容包、双写或后台补算。
