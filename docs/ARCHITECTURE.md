...
主 Agent 可以立即更新 WorldModel，也可以修改未来分析唤醒，但调度不是逃避实验冻结的旁路：只影响认知维护且不产生 Forecast 的唤醒记录 TriggerPlan revision 即可；新增、删除或重排会改变 Forecast 槽或样本选择的触发，必须发布新的触发政策与 producer behavior，不能与修改前 cohort 合并评价。未被当前冻结政策授权的立即 Forecast 仍可作为诊断运行保存，但不能进入 Portfolio。
