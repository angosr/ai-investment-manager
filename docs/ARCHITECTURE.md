...
长期业务对象是一个以结算货币计价的总投资组合，不是 BTC、某个策略或一组互不相干的交易。系统在生存约束内追求现实成本后的长期复利，并以跨市场环境的净增值、回撤和尾部损失共同衡量结果；“稳定盈利”不表示每个周期为正，也不能由一次行情或少量交易证明。当前 10,000 USDT、Binance 和 BTC Spot 都是现役实验边界，不是永久资产域、基准或仓位尺度。
