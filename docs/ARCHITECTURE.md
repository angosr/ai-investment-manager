...
当 Forecast 时域长于采样间隔时，重叠样本可以用于拟合，但不能冒充独立验证证据。开发、验证和盲测边界必须按完整 Outcome 时域 purge；从样本分布推导的 bucket 边界、代表值和基准也只能使用开发带，否则验证/盲测分布已通过标签定义泄漏给模型。有序收益桶使用距离敏感的 ranked proper score 作为候选主排序，同时报告 Brier/校准、预期 bps 对真实连续 bps 的幅度误差与单调性；候选必须先在全部事前固定的非重叠时间相位上不劣于简单基准，再以最差相位的 ranked proper score 作保守排序，完全相同时选更简单的候选。不建立把分数、费用和 PnL 随意加权的综合指标：Forecast 门只否决没有分布增量的因子，唯一下游逻辑账户再独立回答费用后是否值得资本。不得挑表现最好的相位、把重叠行数当成样本量，或用全样本平均值掩盖只在某个时间锚点成立的复杂度。盲测只评价预先选定的唯一候选，揭盲后不得换模型；未来部署可以用训练截止前的全部数据重估已选结构，但该次盲测只能作为历史诊断，真实权限仍只读取后续前瞻结果。
