...
概率配对只回答“谁更接近结果”，不能证明谁能赚钱。费用后生产者对照必须从同一冻结初始账户出发，为每个被比较的 producer behavior 各推进一条由 Evaluation 域拥有的逻辑账户路径：它们共享 Mandate、可投资域、Portfolio、Risk、Execution、产品规则和成本语义，但分别按自身实际 `available_at`、`NO_ESTIMATE`、前序目标与持仓消费 Forecast；任何路径都不得借用正式路径的 Target、仓位、产品投影或成交，也不得读取本次信息截止后的事实。逻辑账户只从公共点时行情和产品事实确定性重建目标、模拟可成交订单、funding、费用与终点权益，不写真实 Venue，也不建立第二套业务账本、调度或 AI 调用；其内容寻址评价制品保存输入身份和完整轨迹，必要时可从公共事实重放。最终同时报告净权益、回撤、尾部损失、换手、费用和相对同风险现金/Reference 的增值，并将差异归因到 Forecast、产品映射、资本选择或执行。单槽目标重放只能诊断某次选择，不能累加后冒充这条路径的费用后业绩；proper score、一次盈利或正式模拟账户的结果也都不能替代该对照。
