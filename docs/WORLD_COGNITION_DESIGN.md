...
同一批点时输入并行推进以下不可变反事实，但只有一条正式模拟资本链：Reference-only；中性 Forecast 加简单程序状态；加入 WorldModel 条件观点。三者使用相同合法工具、风险预算、可成交价格和成本。比较依次回答长期风险溢价是否值得持有、程序状态是否改善再平衡、WorldModel 是否在程序之上增加费用后价值；不能把所有能力打包成一个盈亏数字后猜测贡献。
