...
公共期权 OI、成交量和隐含波动率不能识别买卖方向或交易商净 Gamma，因此不能冒充“期权仓位”；在取得可授权且可回放的仓位或可靠代理合同前保持部分覆盖。就业和通胀的官方实际值同样不能单独构成 surprise；只有发布前已点时保存的市场共识与发布后的官方实际值、修订能够闭合时，才允许声明预期差能力。覆盖率不得靠错误命名提高。
