...
WorldModel 更新时钟与 Forecast 槽时钟是两个领域概念，但共用唯一 TriggerCoordinator：前者来自材料变化和显式复核计划；后者来自合同 cadence，或冻结的材料状态触发政策。事件槽不是新闻到来就任意插入一次 AI，而是由点时 `State/Delta` 和 producer behavior 确定性地产生。槽在生产前冻结仅定时、仅材料或合并来源集合、实际 delta 引用、Outcome、截止和失败处理；合并槽仍只有一次生产义务。触发政策不判断多空，也不读取账户；Heartbeat 只恢复已登记任务、遗漏终态、账户和风险复核，不能制造新样本。
...
触发资格只能读取该时点已经冻结的 State/Delta。它不得读取 WorldModel 的多空观点、账户、当前 Forecast、Portfolio 选择或后续行情；否则“哪些事件值得预测”本身会成为不可评价的隐性 Alpha。每个 cadence 窗口始终拥有一个必须终结的义务；若材料变化落入事前冻结的合并窗口且该义务尚未开始生产，可用一个合并槽同时终结二者，已开始或已完成的槽不得回写。事件去重只作用于同一窗口内等价的材料 delta；组合冲击窗口按上一段的唯一事件规则形成，不再为等等级资产命中复制 delta。仅定时、仅材料与合并槽分别报告覆盖率和预测增量，重叠结果按共同事件聚类。
...
仅定时、仅材料与合并槽分别分层进行配对和报告；合并槽在总体中只计一次，也不能冒充两个独立样本。材料相关样本不能补足仅定时样本的最低数量。既有历史样本不得补生成对照，现役行为若此前没有配对计划，只能从新登记的前瞻评价子集开始积累认知增量证据。
...
仅定时槽评价“在持续、非选择性时点上是否具有一般预测增量”；仅材料槽评价“在冻结触发条件成立时，重新估计是否优于同条件基线”；合并槽评价同一次及时响应是否同时履行了临近 cadence 义务。三类样本分别使用同来源、同触发政策的滚动无条件与市场基线，报告样本量和保守下界；总体按唯一槽身份汇总，不能把合并槽重复计数，也不能把材料相关样本相加后宣称一般 Alpha。共同事件和重叠时域形成依赖簇，而不是多个独立成功。
...
事件触发政策是独立的资本假设：在相同 Forecast、Portfolio、Risk 与执行语义下，对比“仅定时槽”和“定时槽加材料事件槽”的前瞻逻辑账户。后者必须承担自己增加的换手、点差、滑点和错误响应；只证明事件 Forecast 的 proper score 更好，不能证明多一次资本重算值得。改变事件资格、目标映射、去重窗口或响应期限都会产生新的触发行为，不能继承旧政策结果。
...
材料事件槽启用前已经分配和结算的定时样本永久按原行为解释，不追补事件槽、不重写触发来源，也不并入新触发行为的最低样本。当前事件启用合同让每个 cadence 窗口由仅定时或事前合法合并的槽终结，同时按第 5、7 节分别积累仅定时、仅材料和合并样本；改变触发资格、适用资产、时域、合并或去重规则后再次产生新行为身份。无论何种来源，漏过截止的槽都记录 `NO_ESTIMATE`，不得在看到后续行情后补跑。
