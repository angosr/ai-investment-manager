...
当前从认知到 Forecast 的现役路径只有透明 Quant；WorldModel 不直接产生资本 Forecast。未来若新的 AI+Quant 假设先通过与机制时域匹配的离线点时验证，它才可以读取当前 WorldModel、目标相关 State、同槽 Quant prior 和合同，输出结构化收益分布及哪些机制改变了 prior。该候选必须预测与 Quant 完全相同的 Outcome，并保存负向、不确定和 `NO_ESTIMATE`；机制原生时域与合同不匹配且没有前瞻校准投影时，不得靠 Prompt 强行缩放为当前 bucket。
...
当前 Shadow 只保留透明 4h Quant 作为研究 Forecast，业务模拟账户不从其新增风险。已退役 posterior 的配置、运行器、稳定性复算、Dashboard 比较接口和专属测试均已从现役代码删除；不可变数据库事实、迁移与证据制品保留。下一项世界认知实验必须先建立与机制时域匹配的来源无关合同和不读取 WorldModel 的透明基线；没有通过的 Quant prior、没有目标相关机制或没有时域投影支持时，不创建 posterior 调用。首个 24 小时结构性资金流候选已按冻结的两日可用滞后验证：ETF 净流入强度与稳定币七日供给变化的等权分数在总体 RPS、MAE、相关性及跨相位稳定性上失败，blind 未读取，不产生运行实现。该失败不允许通过调权、改滞后或增加相似流量指标续命；未来只有新的正交经济假设才能重新登记。若以后有候选通过，仍只允许一次组合调用、独立 Outcome 和费用后逻辑账户，不增加 Agent、摘要层或资本链。
