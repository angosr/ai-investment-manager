...
这一已登记合同只用于验证世界认知是否产生预测价值，不定义长期交易频率。基础配置失败关闭 `capital`、Context Forecast 和 WorldModel 消融；只有显式 Shadow profile 启用三者，Testnet 不继承该资格。停用期间不产生新槽、资本目标或消融样本；既有不可变事实仍按原合同结算，启用必须由匹配的 Release、行为身份和 EvaluationPlan 明确授权，不能把停用时间伪装成覆盖率：
