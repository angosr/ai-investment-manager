...
旧 v88 输入只有两条事实，尽管事实库当时已有 68 个当前事实，ETF 等连续资金状态因此没有进入模型；这不是世界信息缺失，而是输入压缩失效。现役高密度投影会把全部 14 个当前连续宏观/资金流压缩为可追溯 `StateFeature`，并同时保留一个最新监管候选、一个最新财政操作结果和最近官方日程。基于同一 v88 冻结状态的重放为 17 项有效输入，Packet 投影约 12.4K 字符、完整提示约 13.8K 字符，仍低于 16K 硬上限。省略数量只留在审计，不再发给 AI 冒充现实信息缺口。
