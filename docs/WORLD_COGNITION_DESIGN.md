...
触发资格只能读取该时点已经冻结的 State/Delta。它不得读取 WorldModel 的多空观点、账户、当前 Forecast、Portfolio 选择或后续行情；否则“哪些事件值得预测”本身会成为不可评价的隐性 Alpha。每个 cadence 窗口始终拥有一个必须终结的义务；若材料变化落入事前冻结的合并窗口且该义务尚未开始生产，可用一个合并槽同时终结二者，已开始或已完成的槽不得回写。事件去重只作用于同一窗口内等价的材料 delta。仅定时、仅材料与合并槽分别报告覆盖率和预测增量，重叠结果按共同事件聚类。
