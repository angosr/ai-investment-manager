...
| 产品 | Binance BTC Spot |
| 资本候选 | 现货多头或现金 |
| 预测 | 现役 4h BTC Spot 收益分布和既有自然触发 |
| 成本 | 当前可成交 bid/ask、手续费和滑点 |
| 基线 | 无技巧概率、现金和被动 BTC Spot 暴露 |

保持现役 ForecastContract、producer behavior、输入、成本、门槛和触发不变，直到预登记 cohort 达到停止条件或行为明确失败。负 Forecast 选择现金是完整资本判断：它可以通过避开下跌创造价值，不能做空并不表示预测没有被消费。没有订单、尚无 Outcome 或短期现金状态都不是换产品、放宽门槛或提高频率的理由。

现役 4h 是已经开始的证据合同，不是长期高频授权。它只回答：Context 是否持续输出并按时结算；概率是否优于无技巧基线；WorldModel 是否有增量；长仓/现金选择在完整成本后是否优于现金与被动 Spot。任何正式低频资本权限仍需使用与目标频率匹配的新合同重新验证。
