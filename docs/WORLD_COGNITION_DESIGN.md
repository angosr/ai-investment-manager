...
事件发现只有一条两阶段路径。第一阶段是与真假无关的注意力裁决：事件在 mandate 下的影响足够大就及时唤醒复核，避免重大但尚未核验的冲击被来源等级提前过滤。第二阶段才是证据裁决：来源等级、独立佐证与事件后市场响应不足的线索，只能作为待验证输入和竞争解释，不能单独支持方向、形成材料 Delta/事件 Forecast，或持久化为现役机制引用。两阶段复用同一 Intelligence Event 和同一信息面板，不再建设“新闻分析器”或第二事实库。
