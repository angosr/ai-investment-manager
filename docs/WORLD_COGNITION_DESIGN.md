...
当前从认知到 Forecast 没有现役路径；WorldModel 不直接产生资本 Forecast。未来若新的 Quant 或 AI+Quant 假设先通过与机制时域匹配的离线点时验证和成本下界，它才可以建立前瞻 producer；其中 AI+Quant 读取当前 WorldModel、目标相关 State、同槽 Quant prior 和合同，输出结构化收益分布及哪些机制改变了 prior。该候选必须预测与 Quant 完全相同的 Outcome，并保存负向、不确定和 `NO_ESTIMATE`；机制原生时域与合同不匹配且没有前瞻校准投影时，不得靠 Prompt 强行缩放为当前 bucket。
...
认知增量只由同槽 Quant prior 与唯一 AI+Quant posterior 的前瞻差异识别，不再运行一条“相同 Codex 但删除 WorldModel”的消融链。Quant 是不读取 WorldModel 的可复现对照；posterior 的每次实质偏离必须至少引用一项当前 Packet 中可识别的非市场状态事实或事件，并说明目标 State 如何确认或反驳其传导。纯行情、主动流、OI、funding、仓位、basis 和跨资产价格只能进入 Quant 研究或机制验证，不能单独授权 AI 修改概率。程序把每个机制的结构证据资格冻结在模型输入并在输出审计中强制核对，不能依靠中文措辞门禁。
...
当前 Shadow 不运行 Forecast producer。已退役 posterior 与 4h Quant 的配置、运行任务、Dashboard 现役比较和专属运行路径均已停止；不可变数据库事实、迁移与证据制品保留。4h Quant 即使在最强条件 cell 也无法覆盖确定性的往返手续费下界，继续在线积累 proper score 不会产生资本价值。下一项世界认知实验必须先建立与机制时域匹配的来源无关合同和不读取 WorldModel 的透明基线；没有通过的 Quant prior、没有目标相关机制或没有时域投影支持时，不创建 posterior 调用。首个 24 小时结构性资金流候选已按冻结的两日可用滞后验证：ETF 净流入强度与稳定币七日供给变化的等权分数在总体 RPS、MAE、相关性及跨相位稳定性上失败，blind 未读取，不产生运行实现。该失败不允许通过调权、改滞后或增加相似流量指标续命；未来只有新的正交经济假设才能重新登记。若以后有候选通过，仍只允许一次组合调用、独立 Outcome 和费用后逻辑账户，不增加 Agent、摘要层或资本链。
