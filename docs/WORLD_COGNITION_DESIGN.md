...
容量约束作用于确定性特征、机制验证观测和上一轮认知全部写入后的最终 AI 输入，不能只检查写入前的中间 Packet。Packet 上限还必须为固定 Prompt 与输出契约预留余量，使完整模型输入持续低于运行时上限；直接触发的因果基线无法在该合同内完整表达时属于确定性配置故障，应立即失败并告警，不能按临时故障循环重试。审计身份可以永久留在账本，但不进入模型输入；Release 必须验证最终投影低于运行时容量，运行时拒绝只能是可审计故障，不能充当日常压缩策略。
