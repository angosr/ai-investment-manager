...
长期总组合的推荐默认不是让 AI 直接报目标权重。程序以可复现的长期风险溢价、波动、相关性、趋势、carry、流动性和产品成本形成中性分布；AI 把联合 WorldModel 转为少数有证据引用的情景概率和相对/绝对收益观点；Forecast 再按各来源的真实前瞻校准、不确定性和适用时域确定性收缩与融合。不同原生时域必须先投影到共同资本决策时域，不能把短期事件冲击直接当作长期收益率。没有已证明的 AI 增量时，AI 观点不得影响 official 资本，但可以在预登记 Mock 包络内获得有界实验权重并与无 AI 反事实同口径推进；随着证据增加只调整未来权限，不能用一段自信文字直接扩大仓位。
...
4. 共同 Forecast 与 Portfolio：中性先验、WorldModel 条件观点、共同情景、成本感知目标权重、无交易区间和全账户风险约束。
