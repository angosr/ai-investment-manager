...
AI+Quant posterior 读取当前 WorldModel、目标相关 State、同槽 Quant prior 和合同，输出结构化收益分布及哪些机制改变了 prior；Quant 只读同一时点 State。二者预测完全相同的 Outcome，负向、不确定和 `NO_ESTIMATE` 也都保存。WorldModel 不等待 Quant 先发现机会，Quant 也不对 WorldModel 行使 veto。posterior 只能消费已经由行为合同声明支持该 Outcome 时域的机制投影；机制原生时域更长或更短而没有前瞻校准的投影时，必须改用匹配合同继续研究，不能靠 Prompt 要求模型把叙事强行缩放为当前 bucket。
...
WorldModel 保存组合级完整联合解释，AI+Quant posterior 不重复执行世界认知维护。它读取一个由 `world_model_id` 可追溯的目标投影：联合 synthesis，以及当前相关机制的身份、关系、claim、传导阶段、时域、证据引用和冲突，再结合目标的新鲜 State 与同槽 Quant prior。Quant prior 只代表其冻结制品明确声明并经点时样本外验证的特征，不能把仍仅展示在 State 中的主动流、OI、funding、仓位或 basis 冒充为已经量化。所有可重复计算的市场特征若要提供独立方向信息，必须先成为 Quant 候选并通过同一历史筛选；在此之前，State 只用于核验有外部事实支撑的结构机制是否已经传导、是否失效以及输入是否新鲜，AI 不得把它临场组合成第二套技术模型。完全由市场状态构成的 WorldModel 机制仍可服务认知维护和验证，但在 posterior 投影中明确不具备结构调整资格。全局 Coverage 边界只由 WorldModel 消费一次；目标行情、衍生品或 Quant 输入缺失由确定性构建直接形成对应目标的 `NO_ESTIMATE`。机制的完整因果句、验证合同、连续观测、退休回执、过时事件维护和其他审计元数据仍保存在原 WorldModel，不在 posterior 调用中重复发送。该投影是纯函数，不建立第二份摘要、数据库或认知模型；历史 AI 输入快照保存模型实际看到的投影，并可打开对应完整 WorldModel。
...
同一槽分别保存 prior、posterior、实际生成延迟、完整义务和共同 Outcome；逐目标计算概率差异，一次联合调用仍只形成一个独立时间区间。机制后续是否出现可观察结果直接引用原 WorldModel、Evidence 和 Outcome，不建立第二个机制真相库。若 posterior 相对 Quant 没有 proper-score、校准及费用后资本增量，删除 posterior 或无增量输入；若市场状态确有稳定预测力，把它迁入可回测 Quant 制品，而不是增加更多 AI 提示词或第三个 Agent。
...
当前 Shadow 只保留透明 4h Quant 作为研究 Forecast，业务模拟账户不从其新增风险。下一项世界认知实验必须先建立与机制时域匹配的来源无关合同和不读取 WorldModel 的透明基线；没有通过的 Quant prior、没有目标相关机制或没有时域投影支持时，不创建 posterior 调用。首个 24 小时结构性资金流候选已按冻结的两日可用滞后验证：ETF 净流入强度与稳定币七日供给变化的等权分数在总体 RPS、MAE、相关性及跨相位稳定性上失败，blind 未读取，不产生运行实现。该失败不允许通过调权、改滞后或增加相似流量指标续命；未来只有新的正交经济假设才能重新登记。若以后有候选通过，仍只允许一次组合调用、独立 Outcome 和费用后逻辑账户，不增加 Agent、摘要层或资本链。
