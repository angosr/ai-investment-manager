...
- 动态事件更新 WorldModel；Trigger 显式区分认知-only 弱线索与可建立 Forecast 的材料状态，不能再按新闻类型整体制造事件样本。Forecast 由定时槽或冻结的材料状态事件槽直接唤醒，不依赖 heartbeat 相位。只有新 Forecast 才能改变 Alpha 资本目标，错过的槽位记录 `NO_ESTIMATE`，不事后补跑 AI。
...
主 Agent 显式复核必须绑定已知 Evidence 才能把该线索强制放入本次面板，但普通复核始终只更新 WorldModel；只有冻结触发政策已经标记为材料的官方事件或 Canonical Fact 才能生成材料 Forecast 槽。

代码已经能保存世界认知、Forecast、Portfolio 选择、模拟成交、账户和 Outcome，并区分 Forecast 可用前后的收益。Shadow 现役研究闭环只有 BTC/PAXG 的透明 72h 滚动无条件 prior，以及读取同槽 WorldModel 的唯一 Codex posterior；两者共享 DecisionSlot、信息截止、收益桶和 Outcome。固定 cadence 和 Trigger 已冻结材料资格的官方事件、Canonical Fact 分别形成独立槽；聚合弱线索、市场价格冲击、机制到期和主 Agent 普通复核只更新认知，不凭触发类型制造事件 Forecast。每个正式槽先冻结 prior 与世界更新 Packet，再由同一耐久任务顺序生成同截止 WorldModel 和 posterior；失败或超时以 `NO_ESTIMATE` 闭合，延迟进入真实 `available_at` 与成本评价。首个固定 cadence 前瞻槽为 2026-08-30 00:00 UTC，当前尚无结算结果且没有资本授权。已证伪的 4h AI/Quant、方向资金流、慢趋势、carry 和组合趋势生产路径均已删除，只保留不可变研究结果；现役 72h Quant 候选也全部在 selection 落后于简单 prior，因此没有状态混合或指标拼盘被偷偷启用。
