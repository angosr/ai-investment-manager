...
每个来源月档必须通过同目录官方 SHA-256，标准化观察值固定在结算 60 秒后才可见；下载时间、全部来源摘要与规范化内容共同冻结，旧制品不原地更新。当前唯一活动研究候选 `long-only-dual-trend-28d-sma200-funding-p90-5d-v1` 只在历史研究注册表中存在：它在 28 日动量与 SMA200 同时为正时做多，并在当时可见的 7 日平均资金费率高于此前一年点时分布的第 90 百分位时拒绝开仓。行情下载、`walk-forward` 和后续揭盲均必须传入同一个 `--funding-dataset-id`，数据身份和阈值随完整预登记规格冻结。该候选通过历史门禁前不会接入 Shadow 或生产热路径；失败后从活动代码退役，只保留不可变评价制品。
...
诊断单个运行代次时使用 `--pipeline-version`；跨纯运维发布累计同一 Analyst 行为的前瞻证据时改用运行包记录的 `--analysis-behavior-hash`。二者互斥。行为哈希仅忽略 Pipeline 运行代号，其余完整配置和 Analyst 输入版本任一变化都会产生新作用域；旧版未记录行为哈希的结果不会被事后补入。
