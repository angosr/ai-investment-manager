...
QUANT_CORE_DATABASE_URL='<由部署 Secret 注入>' \
  ...
  --symbol BTCUSDT --replay-start '<起点>' --replay-end '<终点>' \
...
`replay-event-triggers` 与线上 Temporal 协调器复用同一套规则匹配、合并、冷却、到期和单品种小时预算函数，生成不可变外部事件触发带；分析耗时必须显式冻结。它目前不回放 heartbeat、Agent wakeup 或跨品种全局准入，结果会携带这些限制；计划晚于回放起点时也明确标记为反事实诊断。触发带尚未接入交易回测，避免在跨品种预算语义补齐前制造虚假的可成交时钟。
...
基线与门控版本都由同一个 Nautilus 适配器执行，并使用相同数据、下一可成交事件、费用、价差、频率、风控和退出规则。命令从治理事实库读取 `EvaluationPlan.registered_at`，并要求计划中的 `candidate_spec_hash` 同时绑定基础量化策略制品，以及门控周期、最大年龄和置信阈值；调用方不能在看到结果后更换 Q、伪造时间或临时改阈值。输出明确给出增量净收益、回撤变化、交易数变化及路径分叉限制；没有预登记后的同周期预测时拒绝运行。当前刚开始积累多周期带，尚无足够跨环境样本可据此晋级。
