...
首次用未来终点执行 `--register-only`，登记阶段不要求一份与未来窗口无关的占位行情。窗口结束且冻结行情覆盖完整区间后，增加 `--dataset-id '<覆盖评价窗口的数据集>'`，再以其余完全相同参数移除 `--register-only` 运行。基线与门控版本都由同一个 Nautilus 适配器执行，并使用相同数据、下一可成交事件、费用、价差、频率、风控和退出规则。计划快照绑定程序策略、成本/风控制品、预测 Pipeline、品种、周期、权威数据源、固定起止时间、初始权益、点差、Codex 完成延迟上限、门控参数和非重叠样本下限；调用方不能在看到结果后更换 Q、移动终点或临时改阈值。输出明确给出增量净收益、回撤变化、交易数变化、非重叠预测数、证据是否充分及路径分叉限制。当前刚开始积累多周期带，尚无足够跨环境样本可据此晋级。
