...
方向独立的现货/永续 carry 使用单独且受限的研究入口。`fetch-binance-carry-history` 引用同窗口的现货与 SHA 校验资金费率制品，再内容寻址冻结 USD-M 合约成交、标记、指数、溢价日线、结算前 8 小时标记价收盘和当前合约规则；REST 费率必须与官方月档逐条匹配。`carry-walk-forward` 只评价固定的同数量双腿、每腿 50% 权益、月度再平衡策略，并冻结双腿成本、10% 保守维持保证金和 100 bps 单腿失败压力；`carry-blind-evaluate` 复用全局一次性盲区锁，在认领成功前不读取尾窗标签。BTC 开发折的年化收益保守下界为正、最大回撤低于 1%，但其 2025-08 至 2026-08 盲区已被更早候选消费，系统拒绝再次揭示；完全相同的通用规则在预登记 ETH walk-forward 中触发强平边界和保证金门槛，因而失败且未揭盲。该候选没有交易资格，没有接入 Shadow，也没有据此创建生产期货适配器；研究数据和评价合同保留用于结构真正不同且事前登记的新假设。
