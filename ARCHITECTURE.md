...
离线外部事件触发带必须调用线上协调器相同的纯时间规则，并冻结 TriggerPlan、TriggerPolicy、事件数据集和分析耗时假设。单品种回放若没有同时重建跨品种原子调用准入，只能用于触发计划诊断，不能直接生成交易时钟或盈利证据；heartbeat、Agent wakeup、计划生效时间晚于回放窗口等缺口必须作为结构化限制输出，不能藏在说明文字之外。
