...
交易所衍生状态属于独立的点时数值事实，不伪装成新闻事件或现货 K 线。首个受支持类型仅为 Binance USD-M 已结算资金费率：每个月归档及官方 SHA-256、规范化观察值、请求窗口、冻结时间和固定 60 秒结算后可见延迟共同进入内容寻址 Manifest；回测只能读取 `available_at <= signal_at` 的值。归档后来被更换会产生新数据集身份，禁止覆盖旧制品。首个最小候选仅把当时可见的 7 日平均资金费率相对前一年点时分布的第 90 百分位作为既有长周期趋势入场的拥挤否决条件，不把它解释成单独方向信号，也不搜索阈值。premium/basis、持仓量和任意第三方指标在有独立假设前不进入契约；该候选未通过预登记历史门禁前，不增加生产热路径采集器，失败则从活动注册表退役而不继续堆叠过滤器。
...
Pipeline 版本仍是 Temporal 与运行状态的代次边界，每次代码发布可以递增；前瞻方向评价另以 `analysis_behavior_hash` 标识可合并的 Analyst 行为。该哈希只归一化 Pipeline 运行代号，其余完整类型化配置与 Analyst 输入版本全部参与计算，并同时冻结在运行包、唯一 Codex Attempt 和到期结果中。因此纯运维发布不会把样本统计归零，模型、提示输入、面板、预测或任何其他配置变化仍会自动切断证据；旧结果不允许事后推断或补写该哈希。
