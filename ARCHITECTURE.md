...
3. 同一条已冻结决策带可以喂给多个**预登记**的确定性 Shadow 变体，例如价格基线、价格基线加 AI 否决、AI 倾向加不同但已冻结的经济性门槛。各变体从 Codex 实际完成后的首个可成交事件开始，复用生产的候选校验、Composer、Frequency、Risk、成本、撮合和退出规则，并使用隔离虚拟账户。模型只运行一次，交易规则可以高速离线回放多次。
4. 变体规格及哈希必须在对应结果成熟前登记；开发窗口可以筛选，评价窗口和一次性盲测只运行冻结规格。禁止看到标签后不断生成门槛并把最佳结果称为样本外。
