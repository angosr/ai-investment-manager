...
def test_v7_keeps_ambiguous_local_macro_as_background_only() -> None:
    observed_at = datetime(2026, 8, 20, 8, 31, tzinfo=UTC)
    legacy = EventNormalizer(
        version="trendradar-collector-v6",
        universe=("BTCUSDT", "ETHUSDT"),
    )
    ...
    titles = (
        "Hong Kong composite CPI YoY 1.7% in July",
        "Composite CPI (YoY) (Jul)",
        "瑞典央行：评估今年晚些时候加息的可能性依然存在。",
        "澳大利亚失业率意外上升，市场降低加息预期。",
    )

    for index, title in enumerate(titles):
        item = RawIntelligenceItem(
            source_item_id=f"local-macro-{index}",
            source="wire",
            event_time=observed_at,
            observed_at=observed_at,
            title=title,
        ...
        old_event = legacy.normalize(item)
        new_event = candidate.normalize(item)
        assert old_event is not None and old_event.relevance == Decimal("0.85")
        assert new_event is not None and new_event.relevance == Decimal("0.50")
        assert new_event.impact == Decimal("0.495")


def test_v7_high_impact_requires_identifiable_systemic_shock() -> None:
    observed_at = datetime(2026, 8, 20, 8, 31, tzinfo=UTC)
    normalizer = EventNormalizer(
        version="trendradar-collector-v7",
        universe=("BTCUSDT", "ETHUSDT"),
    ...
    titles = (
        "US CPI rises above consensus",
        "Federal Reserve rate decision raises rates",
        "ECB rate cut surprises markets",
    ...
    for index, title in enumerate(titles):
        event = normalizer.normalize(
            RawIntelligenceItem(
                source_item_id=f"systemic-shock-{index}",
                source="wire",
                event_time=observed_at,
                observed_at=observed_at,
                title=title,
